...
l = 61:5:71; %60.5/12; % wheelbase (ft)
...
for whlbse_index = 1:1:length(l)
    ...
    for  i = 1:1:length(velocity) % for each velocity
        ...
        wf = wf-Ax*cg*WS/l(whlbse_index); 
        wr = wr+Ax*cg*WS/l(whlbse_index);
        % recalculate camber angles due to pitch
        IA_f = -l(whlbse_index)*12*sin(pitch)/2*IA_gainf + IA_0f;
        IA_r = l(whlbse_index)*12*sin(pitch)/2*IA_gainr + IA_0r;
        ...
        while AX_diff>0
            ...
            wf = (WF+DF*CoP)/2;
            wr = (WR+DF*(1-CoP))/2;
            wf = wf-Ax*cg*WS/l(whlbse_index)/24;
            wr = wr+Ax*cg*WS/l(whlbse_index)/24;
            IA_f = -l(whlbse_index)*12*sin(pitch)/2*IA_gainf + IA_0f;% - KPIf*(1-cos(deltaf)) + casterf*sin(deltaf);
            IA_r = l(whlbse_index)*12*sin(pitch)/2*IA_gainr + IA_0r;% - KPIr*(1-cos(deltar)) + casterf*sin(deltar);
            FZ_vals = [-250:1:-50];
            sl = [0:.01:.11];
            for k = 1:length(sl)
                fxf(k) = fnval([sl(k);-wf;rad2deg(-IA_f)],full_send_x)*sf_x;
    ...
    for turn = 1:1:length(radii)
        % first define your vehicle characteristics:
            a = l(whlbse_index)*(1-WDF);
            b = l(whlbse_index)*WDF;
            R = radii(turn);
            ...
            ar = rad2deg(a_r);
            steer = rad2deg(delta);
            UG = rad2deg(delta-l(whlbse_index)/R)*32.2/AY;
            Ugradient(turn) = UG;
    ...
    frontF(3,:) = [WF/2 + Cl*VX_max^2*CoP/2 - WF*AX_max*cg/l(whlbse_index)/2 , WF/2 + Cl*VX_min^2*CoP/2 - WF*AX_min*cg/l(whlbse_index)/2, WF/2 + Cl*VY_max^2*CoP/2 + WF*AY_max*cg/tw/2];
    rearF(3,:) = [WR/2 + Cl*VX_max^2*(1-CoP)/2 + WR*AX_max*cg/l(whlbse_index)/2 , WR/2 + Cl*VX_min^2*(1-CoP)/2 + WR*AX_min*cg/l(whlbse_index)/2, WR/2 + Cl*VY_max^2*(1-CoP)/2 + WR*AY_max*cg/tw/2];
    frontF(2,:) = [0 0 (WF/2+WF*AY_max*cg/tw/2)*AY_max];
    rearF(2,:) = [0 0 (WR/2+WR*AY_max*cg/tw/2)*AY_max];
    frontF(1,:) = [0 -(WF/2 -WF*AX_min*cg/l(whlbse_index)/2)*AX_min 0];
    rearF(1,:) = [W*AX_max/2 -(WR/2 +WR*AX_min*cg/l(whlbse_index)/2)*AX_min 0];
